window.database = {
    ...
    "15-nodejs-aws-s3-html": {
      ...
      "category": "",
      "content": "Node.js - Simple S3 examples within async functionsA simple get object example within an async function.(async function() {    const AWS = require(\"aws-sdk\");  const s3 = new AWS.S3();    let results;  try {        const s3Result = await s3.getObject({      Bucket: \"bucketname\",      Key: \"key\"    }).promise();    results = s3Result.Body.toString(\"utf8\");      } catch (e) {        console.log(`Could not retrieve from S3: ${e.message}`);    results = \"\";      }  console.log(results);  })();Put object example.(async function() {    const AWS = require(\"aws-sdk\");  const s3 = new AWS.S3();    try {        await s3.putObject({      Body: \"Some text\",      Bucket: \"bucketname\",      Key: \"key\",      ContentType: \"text/plain\"    }).promise();        console.log(\"Saved to S3\");      } catch (e) {        console.log(`Could not save to S3: ${e.message}`);      }  })();Delete object example.(async function() {    const AWS = require(\"aws-sdk\");  const s3 = new AWS.S3();    try {        await s3.deleteObject({      Bucket: \"bucketname\",      Key: \"key\"    }).promise();        console.log(\"Deleted object in S3\");      } catch (e) {        console.log(`Could not delete object in S3: ${e.message}`);      }  })();",
